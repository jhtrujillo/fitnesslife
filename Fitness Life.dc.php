...
<div style="background:oklch(93% .004 270);min-height:100vh;overflow-x:hidden">
  ...
  <sc-if value="{{isCategoria}}" hint-placeholder-val="{{false}}">
    <main style="padding-bottom:100px">
      <section style="position:relative;height:52vh;min-height:380px;overflow:hidden">
        <img src="assets/hero_hoist.png" data-src="{{currentCategory.img}}" alt="{{currentCategory.name}}" ref="{{heroImgRef}}" style="position:absolute;inset:0;width:100%;height:100%;object-fit:cover"/>
        <div style="position:absolute;inset:0;background:linear-gradient(to top, oklch(14% .005 270) 5%, oklch(8% .005 270 / .3) 70%)"></div>
        <div style="position:relative;z-index:5;max-width:1280px;margin:0 auto;padding:0 24px;height:100%;display:flex;flex-direction:column;justify-content:flex-end;padding-bottom:48px">
          <div style="font-size:12px;letter-spacing:0.28em;color:oklch(65% .24 27);font-weight:700;text-transform:uppercase;margin-bottom:12px">{{currentCategory.tag}}</div>
          <h1 style="font-family:Oswald,sans-serif;font-size:clamp(36px,6vw,64px);font-weight:700;margin:0;color:oklch(98% 0 0)">{{currentCategory.name}}</h1>
        </div>
      </section>
      <section style="max-width:1000px;margin:0 auto;padding:64px 24px">
        <p style="font-size:17px;line-height:1.7;color:oklch(80% .01 270);margin:0 0 48px">{{currentCategory.longDesc}}</p>
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:24px;margin-bottom:56px">
          <sc-for list="{{currentCategory.features}}" as="f" hint-placeholder-count="4">
            <div style="border:1px solid oklch(83% .006 270);padding:24px;background:oklch(98% 0 0)">
              <h3 style="font-size:15px;font-weight:600;margin:0 0 8px;color:oklch(20% .005 270)">{{f.title}}</h3>
              <p style="font-size:13px;color:oklch(42% .01 270);line-height:1.6;margin:0">{{f.desc}}</p>
            </div>
          </sc-for>
        </div>

        <h2 style="font-family:Oswald,sans-serif;font-size:28px;font-weight:700;margin:0 0 24px;color:oklch(20% .005 270)">Equipos Disponibles</h2>
        <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(250px,1fr));gap:24px;margin-bottom:56px">
          <sc-for list="{{categoryProducts}}" as="prod">
            <div style="border:1px solid oklch(88% .006 270);border-radius:12px;overflow:hidden;background:white;transition:transform .3s ease, box-shadow .3s ease" style-hover="transform:translateY(-5px);box-shadow:0 12px 24px -10px oklch(20% .01 270 / .2)">
              <div style="height:220px;background:oklch(97% .004 270);padding:24px;display:flex;align-items:center;justify-content:center;position:relative">
                <img loading="lazy" src="{{prod.image}}" alt="{{prod.name}}" style="max-width:100%;max-height:100%;object-fit:contain"/>
              </div>
              <div style="padding:20px">
                <div style="font-size:11px;color:oklch(58% .22 25);font-weight:700;text-transform:uppercase;margin-bottom:6px">{{prod.series}}</div>
                <h4 style="font-family:Oswald,sans-serif;font-size:18px;margin:0 0 10px;color:oklch(20% .005 270);line-height:1.3">{{prod.name}}</h4>
                <a href="#" onClick="{{goToContacto}}" style="display:inline-block;font-size:12px;font-weight:700;color:oklch(58% .22 25);text-transform:uppercase;letter-spacing:0.05em">Cotizar →</a>
              </div>
            </div>
          </sc-for>
          <sc-if value="{{noProducts}}" hint-placeholder-val="{{false}}">
             <p style="color:oklch(42% .01 270);grid-column:1/-1">En este momento no hay equipos clasificados en esta categoría.</p>
          </sc-if>
        </div>

        <div style="background:oklch(98% 0 0);border:1px solid oklch(83% .006 270);padding:40px;display:flex;align-items:center;justify-content:space-between;gap:24px;flex-wrap:wrap">
          <div>
            <h3 style="font-family:Oswald,sans-serif;font-size:22px;font-weight:600;margin:0 0 8px">¿Listo para equipar tu espacio?</h3>
            <p style="color:oklch(42% .01 270);font-size:14px;margin:0">Marcas disponibles: Life Fitness, Precor, Hoist, Strength Design.</p>
          </div>
          <a href="#" onClick="{{goToContacto}}" style="background:linear-gradient(135deg, oklch(58% .22 25), oklch(65% .24 27));color:white;padding:16px 30px;font-weight:700;font-size:12px;letter-spacing:0.08em;text-transform:uppercase;white-space:nowrap">Solicita tu cotización</a>
          </div>
      </section>
    </main>
  </sc-if>
      ...
      <section style="max-width:1280px;margin:0 auto;padding:40px 24px">
        <div style="display:flex;flex-wrap:wrap;gap:12px;margin-bottom:40px;justify-content:center">
          <sc-for list="{{catalogoFilters}}" as="f">
            <button onClick="{{f.onClick}}" style="background:{{f.bg}};color:{{f.color}};border:1px solid {{f.border}};padding:10px 20px;border-radius:999px;font-size:13px;font-weight:600;letter-spacing:0.04em;text-transform:uppercase;cursor:pointer;transition:all .2s ease">{{f.label}}</button>
          </sc-for>
        </div>
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(250px,1fr));gap:24px">
          <sc-for list="{{catalogoProducts}}" as="prod">
            <div style="border:1px solid oklch(88% .006 270);border-radius:12px;overflow:hidden;background:white;transition:transform .3s ease,box-shadow .3s ease" style-hover="transform:translateY(-5px);box-shadow:0 12px 24px -10px oklch(20% .01 270 / .2)">
              <div style="height:220px;background:oklch(97% .004 270);padding:24px;display:flex;align-items:center;justify-content:center">
                <img loading="lazy" src="{{prod.image}}" alt="{{prod.name}}" style="max-width:100%;max-height:100%;object-fit:contain"/>
              </div>
              <div style="padding:20px">
                <div style="font-size:11px;color:oklch(58% .22 25);font-weight:700;text-transform:uppercase;margin-bottom:6px">{{prod.series}}</div>
          ...
          <sc-if value="{{noCatalogoProducts}}" hint-placeholder-val="{{false}}">
             <p style="color:oklch(42% .01 270);grid-column:1/-1;text-align:center;padding:40px">No se encontraron productos para este filtro.</p>
          </sc-if>
        </div>
      </section>
    </main>
    ...
    <main style="max-width:1280px;margin:0 auto;padding:80px 24px 120px">
      ...
      <div style="display:grid;grid-template-columns:1fr 1.3fr;gap:48px">
        <div style="display:flex;flex-direction:column;gap:28px">
          ...
          <div style="height:180px;background:repeating-linear-gradient(135deg, oklch(90% .004 270), oklch(90% .004 270) 10px, oklch(83% .006 270) 10px, oklch(83% .006 270) 20px);display:flex;align-items:center;justify-content:center;color:oklch(42% .01 270);font-family:monospace;font-size:12px;border:1px solid oklch(83% .006 270)">[ mapa de cobertura ]</div>
        </div>
...
class Component extends DCLogic {
  ...
  setCatalogoFilter = (filter) => (e) => {
    if (e) e.preventDefault();
    this.setState({ catalogoFilter: filter });
  };

  goToCategory = (slug) => {
    window.history.pushState(null, '', '#categoria=' + slug);
    this.setState({ page: 'categoria', categorySlug: slug }, () => {
      setTimeout(() => window.scrollTo(0, 0), 10);
      setTimeout(() => window.scrollTo(0, 0), 100);
    });
  };
  ...
  renderVals() {
    ...
    const categories = Object.values(CATEGORIES).filter(c => ['elipticas', 'bicicletas', 'trotadoras', 'pesas'].includes(c.slug)).map((c, i) => ({
      ...
      revealOpacity: this.state.categoriesVisible ? 1 : 0,
    ...
    const categoryProducts = (this.state.allProducts || []).filter(p => {
      ...
      if (image && !image.startsWith('http') && !image.startsWith('v1/cotizaciones/')) { image = 'v1/cotizaciones/' + image; } return { ...p, image };
    });
    ...
    const catalogoFilters = filterDefs.map(f => {
      ...
      return {
        ...
        bg: active ? 'oklch(58% .22 25)' : 'white',
        color: active ? 'white' : 'oklch(30% .008 270)',
        border: active ? 'oklch(58% .22 25)' : 'oklch(80% .008 270)'
      };
    });

    const catalogoProducts = (this.state.allProducts || []).filter(p => {
      const f = this.state.catalogoFilter;
      if (f === 'todos') return true;
      const name = (p.name || '').toLowerCase();
      const series = (p.series || '').toLowerCase();
      const search = name + ' ' + series;
      if (f === 'elipticas') return search.includes('elliptical') || search.includes('elíptica') || search.includes('eliptica') || search.includes('cross trainer');
      if (f === 'trotadoras') return search.includes('treadmill') || search.includes('trotadora') || search.includes('caminadora') || search.includes('run');
      if (f === 'bicicletas') return search.includes('bike') || search.includes('bicicleta') || search.includes('spin') || search.includes('cycle') || search.includes('stair') || search.includes('escalera') || search.includes('climb') || search.includes('step');
      if (f === 'accesorios') return search.includes('mancuerna') || search.includes('barra') || search.includes('disco') || search.includes('accesorio') || search.includes('grip') || search.includes('kettlebell') || search.includes('banda') || search.includes('peso');
      if (f === 'pesas') return !search.includes('elliptical') && !search.includes('eliptica') && !search.includes('treadmill') && !search.includes('trotadora') && !search.includes('caminadora') && !search.includes('bike') && !search.includes('bicicleta') && !search.includes('spin') && !search.includes('cycle') && !search.includes('stair') && !search.includes('escalera') && !search.includes('climb') && !search.includes('accesorio') && !search.includes('mancuerna');
      return false;
    }).map(p => {
      let image = p.img || '';
      if (p.media_json && p.media_json.length > 0 && p.media_json[0].url) {
    ...
    const solutionsData = [
      ...
      return {
        ...
        revealOpacity: this.state.solucionesVisible ? 1 : 0,
        revealTransform: this.state.solucionesVisible ? 'translateY(0)' : 'translateY(38px)',
    ...
    const processStepsData = [
      ...
      ...s,
      revealOpacity: this.state.processVisible ? 1 : 0,
    ...
    return {
      ...
      categoriesHeaderOpacity: this.state.categoriesVisible ? 1 : 0,
      categoriesHeaderTransform: this.state.categoriesVisible ? 'translateY(0)' : 'translateY(28px)',
      solucionesRef: this.setSectionRef('solucionesVisible'),
      solucionesHeaderOpacity: this.state.solucionesVisible ? 1 : 0,
      solucionesHeaderTransform: this.state.solucionesVisible ? 'translateY(0)' : 'translateY(28px)',
      ...
      processHeaderOpacity: this.state.processVisible ? 1 : 0,
      processHeaderTransform: this.state.processVisible ? 'translateY(0)' : 'translateY(28px)',
      marcasRef: this.setSectionRef('marcasVisible'),
      marcasHeaderOpacity: this.state.marcasVisible ? 1 : 0,
      marcasHeaderTransform: this.state.marcasVisible ? 'translateY(0)' : 'translateY(28px)',
      clientesRef: this.setSectionRef('clientesVisible'),
      clientesHeaderOpacity: this.state.clientesVisible ? 1 : 0,
      clientesHeaderTransform: this.state.clientesVisible ? 'translateY(0)' : 'translateY(28px)',
      whyUsRef: this.setSectionRef('whyUsVisible'),
      whyUsHeaderOpacity: this.state.whyUsVisible ? 1 : 0,
      whyUsHeaderTransform: this.state.whyUsVisible ? 'translateY(0)' : 'translateY(28px)',
      whyUsBannerOpacity: this.state.whyUsVisible ? 1 : 0,
      whyUsBannerTransform: this.state.whyUsVisible ? 'translateY(0)' : 'translateY(40px)',
      goHome: this.goHome,
